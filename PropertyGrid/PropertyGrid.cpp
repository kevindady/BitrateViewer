...
void CPropertyGrid::SetTrueFalseStrings(wstring strTrue, wstring strFalse)
{
  m_strTrue = strTrue;
  m_strFalse = strFalse;
}

void CPropertyGrid::SetOkCancelStrings(wstring strOk, wstring strCancel)
{
  m_strOk = strOk;
  m_strCancel = strCancel;
}

void CPropertyGrid::SetDateTimeStrings(wstring strDate, wstring strTime)
{
  m_strDate = strDate;
  m_strTime = strTime;
}

void CPropertyGrid::SetUndefinedString(wstring strUndefined)
{
  m_strUndefined = strUndefined;
}

void CPropertyGrid::SetEmptyString(wstring strEmpty)
{
  m_strEmpty = strEmpty;
}
...
bool CPropertyGrid::CItem::operator==(const wstring& name) const
{
  return m_name == name;
}
...
HSECTION CPropertyGrid::AddSection(wstring title, bool collapsed, HSECTION after)
{
  // build it
  CSection section;
...
HITEM CPropertyGrid::AddItem(HSECTION hs, EItemType type, wstring name, void* pValue, bool editable, bool undefined, HITEM after)
{
  ...
  else if (type == IT_STRING || type == IT_TEXT || type == IT_FILE || type == IT_FOLDER) item.m_strValue = *(wstring*)pValue;
...
HITEM CPropertyGrid::AddCustomItem(HSECTION section, wstring name, ICustomItem* pItem, bool editable, HITEM after)
{
  pItem->m_pGrid = this;
  return AddItem(section, IT_CUSTOM, name, pItem, editable, false, after);
}

HITEM CPropertyGrid::AddStringItem(HSECTION section, wstring name, wstring value, bool editable, HITEM after)
{
  return AddItem(section, IT_STRING, name, &value, editable, false, after);
}

HITEM CPropertyGrid::AddTextItem(HSECTION section, wstring name, wstring value, bool editable, HITEM after)
{
  return AddItem(section, IT_TEXT, name, &value, editable, false, after);
}

HITEM CPropertyGrid::AddIntegerItem(HSECTION section, wstring name, int value, wstring format, bool editable, bool undefined, HITEM after)
{
...
HITEM CPropertyGrid::AddDoubleItem(HSECTION section, wstring name, double value, wstring format, bool editable, bool undefined, HITEM after)
{
...
HITEM CPropertyGrid::AddComboItem(HSECTION section, wstring name, const vector<wstring>& values, int cur, bool editable, bool undefined, HITEM after)
{
...
HITEM CPropertyGrid::AddBoolItem(HSECTION section, wstring name, bool value, bool editable, bool undefined, HITEM after)
{
  return AddItem(section, IT_BOOLEAN, name, &value, editable, undefined, after);
}

HITEM CPropertyGrid::AddDateItem(HSECTION section, wstring name, COleDateTime value, wstring format, bool editable, bool undefined, HITEM after)
{
...
HITEM CPropertyGrid::AddDateTimeItem(HSECTION section, wstring name, COleDateTime value, wstring format, bool editable, bool undefined, HITEM after)
{
...
HITEM CPropertyGrid::AddFileItem(HSECTION section, wstring name, wstring value, wstring filter, bool editable, HITEM after)
{
...
HITEM CPropertyGrid::AddFolderItem(HSECTION section, wstring name, wstring value, wstring title, bool editable, HITEM after)
{
...
HITEM CPropertyGrid::AddColorItem(HSECTION section, wstring name, COLORREF value, bool editable, bool undefined, HITEM after)
{
  return AddItem(section, IT_COLOR, name, &value, editable, undefined, after);
}

HITEM CPropertyGrid::AddFontItem(HSECTION section, wstring name, LOGFONT value, bool editable, bool undefined, HITEM after)
{
...
bool CPropertyGrid::GetItemValue(HITEM item, wstring& strValue) const
{
  // get the item
...
bool CPropertyGrid::SetItemValue(HITEM item, const wstring strValue)
{
  // get the item
  CItem* pItem = FindItem(item);
  if (pItem == NULL) return false;
...
void CPropertyGrid::DrawItem(CDC& dc, int w, int x, int y, vector<CItem>::iterator& it)
{
  ...
  else
  {
    ...
    // now draw text
    wstring strValue = it->m_strValue;
    switch (it->m_type)
    {
    case IT_TEXT:
      {
    ...
    case IT_FOLDER:
      {
        TCHAR szBuffer[1024];
		_tcsncpy_s(szBuffer, strValue.c_str(), 1024);
...
LRESULT CPropertyGrid::OnEditChanged(WPARAM wParam, LPARAM lParam)
{
  ...
  if (pItem)
  {
    if (pItem->m_type == IT_STRING)
    {
      pItem->m_strValue = wstring((TCHAR*)wParam);
      pItem->m_undefined = false;
      GetOwner()->SendMessage(WM_PG_ITEMCHANGED, pItem->m_id);
      DeleteEditControl();
      Invalidate();
    ...
    {
      if (pItem->m_pCustom->OnItemEdited(wstring((TCHAR*)wParam)))
        GetOwner()->SendMessage(WM_PG_ITEMCHANGED, pItem->m_id);
      DeleteEditControl();
      Invalidate();
    }
